...
import { gregSourceToHebrewString, formatRelativePostedDate } from '../utils/dateConverter';
...
export const AdminDashboardPage: React.FC<PageProps> = ({ setCurrentPage }) => {
  ...
  const confirmUserDelete = (targetUser: User) => {
    setModalContent({
      ...
      message: `האם אתה בטוח שברצונך למחוק את המשתמש ${targetUser.fullName || targetUser.email}? פעולה זו תמחק את חשבון ה-Auth שלו ואת מסמך הפרופיל ב-Firestore. מחיקת נתונים מקושרים (משרות, צ'אטים) דורשת יישום Firebase Functions.`,
  ...
  if (accessDenied) {
    return (
      <div className="text-center p-10">
        <h1 className="text-2xl font-bold text-red-600">גישה נדחתה</h1>
    ...
    <div className="space-y-8 p-2 sm:p-0">
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <StatCard title="סהכ שיחות צ'אט" value={stats.totalChatThreads} icon={<ChatBubbleLeftEllipsisIcon className="w-8 h-8"/>} />
        </div>
      )}
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-3 text-center font-medium text-gray-500">יכולת צ'אט</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {allUsers.map(u => (
                <tr key={u.id} className={`${u.isBlocked ? 'bg-red-50 opacity-70' : (u.role === 'admin' ? 'bg-blue-50' : '')}`}>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap text-center">
                     <Button size="sm" variant={u.canChat === false ? "danger" : "outline"} onClick={() => handleUserAction(() => authService.toggleUserChatStatusAdmin(u.id, !(u.canChat ?? true)), `יכולת צ'אט למשתמש ${u.email} ${!(u.canChat ?? true) ? 'הופעלה' : 'נחסמה'}.`)} className="!px-2 !py-1">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {allJobs.map(job => (
                <tr key={job.id} className={`${job.isFlagged ? 'bg-yellow-50' : ''}`}>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap">{gregSourceToHebrewString(job.postedDate)}</td>
                  <td className="px-3 py-3 whitespace-nowrap text-center">
      ...
      {/* Chat Threads Table */}
       <section className="bg-white p-4 sm:p-6 rounded-xl shadow-lg">
        <h2 className="text-xl font-semibold text-royal-blue mb-4">שיחות צ'אט ({allChatThreads.length})</h2>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 text-sm">
             <thead className="bg-gray-50">
                <tr>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {allChatThreads.map(thread => (
                <tr key={thread.id}>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap">
                    {thread.lastMessage ? `${thread.lastMessage.text.substring(0,30)}... (${formatRelativePostedDate(thread.lastMessage.timestamp)})` : 'אין הודעות'}
                  </td>
                  <td className="px-3 py-3 whitespace-nowrap">
