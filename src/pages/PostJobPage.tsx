...
import { getTodayGregorianISO, gregSourceToHebrewString } from '../utils/dateConverter';
import { USE_FIREBASE_BACKEND } from '../../config';
import * as MockJobService from '../../services/jobService';
import * as FirebaseJobService from '../services/jobService';

const jobService = USE_FIREBASE_BACKEND ? FirebaseJobService : MockJobService;
...
const fieldLabels: Record<string, string> = {
  ...
  allowSiteMessages: "אפשר פנייה בהודעות האתר"
};
...
export const PostJobPage: React.FC<PageProps> = ({ setCurrentPage, pageParams }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-6 sm:p-8 rounded-xl shadow-2xl my-8">
      ...
      <div className="bg-light-blue/60 p-4 rounded-lg mb-8 flex items-center text-royal-blue shadow-sm border border-royal-blue/30">
        <ClockIcon className="w-8 h-8 ml-3 rtl:mr-3 rtl:ml-0 text-royal-blue flex-shrink-0" />
        <p className="text-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-6" noValidate>
        ...
        <fieldset className="p-4 border border-gray-200 rounded-md">
            <legend className={royalBlueLegendClassName}>תשלום</legend>
        ...
        <fieldset className="p-4 border border-gray-200 rounded-md">
            ...
            {formData.contactInfoSource === 'other' && (
                <div className="space-y-4 mt-3 p-3 border rounded-md bg-light-blue/20 animate-fade-in-down">
            ...
            {formData.contactInfoSource === 'currentUser' && user && (
                 <div className="mt-3 p-3 border rounded-md bg-green-50 animate-fade-in-down">
             ...
             {formData.contactInfoSource === 'anonymous' && (
                <div className="space-y-4 mt-3 p-3 border rounded-md bg-yellow-50/70 animate-fade-in-down">
                    <p className="text-sm text-yellow-800 font-semibold text-right">
            ...
            <fieldset className="mt-4">
                ...
                <CheckboxGroup 
                    ...
                    selectedValues={new Set(Object.entries(formData.preferredContactMethods || {}).filter(([, val]) => val === true && ['phone', 'whatsapp', 'email'].includes(val[0])).map(([key]) => key))} 
                ...
                >
                    <span className="text-sm text-dark-text mr-3 rtl:ml-3 rtl:mr-0">אפשר פנייה בהודעות האתר</span>
                    <div className="relative inline-block w-10 align-middle select-none">
                        <input
                            type="checkbox"
             ...
             {errors.preferredContactMethods && <p className="mt-1 text-xs text-red-600 text-right">{errors.preferredContactMethods}</p>}
        </fieldset>

        {globalErrorSummary && <p className="mb-4 text-center text-sm text-red-600 bg-red-100 p-3 rounded-md">{globalErrorSummary}</p>}
        {errors.form && <p className="text-center text-sm text-red-600 bg-red-100 p-3 rounded-md mt-2">{errors.form}</p>}

        <div className="bg-yellow-50 border border-yellow-300 p-4 rounded-md mt-6 mb-4 flex items-start text-yellow-800 shadow-sm">
            <LightBulbIcon className="w-7 h-7 ml-3 rtl:mr-3 rtl:ml-0 text-yellow-700 flex-shrink-0 mt-0.5" />
            <p className="text-sm">
