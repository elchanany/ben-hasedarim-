                        ...
                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                            ...
                            <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-4 rounded-xl border border-indigo-100 mt-4">
                                ...
                                <div className="space-y-3">
                                    ...
                                    {/* Tzintuk (Phone Call) notifications */}
                                    <label className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200 cursor-pointer hover:border-blue-300 transition-all flex-wrap">
                                        <div className="flex items-center gap-3 flex-1 min-w-[200px]">
                                            <input
                                                type="checkbox"
                                                checked={alertFormData.deliveryMethods?.tzintuk ?? false}
                                                onChange={(e) => handleValueChange('deliveryMethods', { ...alertFormData.deliveryMethods, tzintuk: e.target.checked })}
                                                className="w-5 h-5 text-royal-blue rounded border-gray-300 focus:ring-royal-blue"
                                            />
                                            <div className="flex-1">
                                                <span className="font-medium text-gray-800 flex items-center gap-2">
                                                    📞 צינתוק (שיחה טלפונית)
                                                    <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-bold">חדש!</span>
                                                </span>
                                                <p className="text-xs text-gray-500">תקבל שיחה קצרה כשיש משרות חדשות - גם בלי אינטרנט!</p>
                                            </div>
                                        </div>

                                        {alertFormData.deliveryMethods?.tzintuk && (
                                            <div className="w-full animate-fade-in mt-2" onClick={(e) => e.stopPropagation()}>
                                                <Input
                                                    name="alertTzintukPhone"
                                                    value={alertFormData.alertTzintukPhone || ''}
                                                    onChange={handleAlertFormChange}
                                                    placeholder="050-1234567"
                                                    containerClassName="mb-0"
                                                    inputClassName="text-sm"
                                                />
                                                <p className="text-xs text-gray-400 mt-1">הזן מספר טלפון לקבלת צינתוקים</p>
                                            </div>
                                        )}
                                    </label>
