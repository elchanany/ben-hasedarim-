...
const ChatThreadListItem: React.FC<{
  ...
  return (
    <li
      onClick={() => onClick(thread.id, otherParticipant.name || otherParticipant.displayName || "משתתף", thread.jobTitle, thread.jobId)}
      ...
      role="button"
      tabIndex={0}
      aria-label={`פתח שיחה עם ${otherParticipant.name || otherParticipant.displayName || "משתתף"}${thread.jobTitle ? ` לגבי ${thread.jobTitle}` : ''}. ${unreadCount > 0 ? `${unreadCount} הודעות חדשות.` : ''}`}
    >
      ...
      <div className="flex-grow min-w-0">
        <div className="flex justify-between items-center">
          <h4 className={`text-base sm:text-lg truncate ${unreadCount > 0 ? 'text-deep-pink' : 'text-royal-blue'}`}>
            {otherParticipant.name || otherParticipant.displayName || "משתתף"}
          </h4>
