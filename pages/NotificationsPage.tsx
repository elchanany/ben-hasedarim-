...
const ChatThreadListItem: React.FC<{
  ...
  return (
    <li
      ...
      className={`p-3 sm:p-4 rounded-lg border flex items-center space-x-3 rtl:space-x-reverse cursor-pointer transition-colors duration-150 hover:bg-light-blue
        ...
        <div className="flex justify-between items-center">
          ...
          {thread.lastMessage && (
            <span className="text-xs text-gray-500 flex-shrink-0 ml-2 rtl:mr-2 rtl:ml-0">
              {formatRelativePostedDate(thread.lastMessage.timestamp)}
            </span>
          )}
        </div>
        {thread.jobTitle && (
...
export const NotificationsPage: React.FC<PageProps> = ({ setCurrentPage, pageParams }) => {
  ...
  useEffect(() => {
    if (pageParams?.tab !== activeTab) {
        setCurrentPage('notifications', { tab: activeTab });
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 py-4 sm:py-8 px-2 sm:px-0">
      ...
      <div className="flex border-b border-gray-200 bg-white sm:rounded-t-lg shadow-sm">
        <TabButton 
            label="התראות מערכת" 
                  ...
                  <li key={notif.id} className={`p-3 sm:p-4 rounded-lg border ${notif.isRead ? 'bg-gray-50 border-gray-200' : 'bg-light-blue border-royal-blue shadow-sm'}`}>
                    <div className="flex justify-between items-start">
                      <div className="flex-grow">
                        ...
                        <p className="text-xs text-gray-400 mt-1">{formatRelativePostedDate(notif.createdAt)}</p>
                      </div>
                  ...
                  <div key={alert.id} className="p-4 border rounded-lg bg-gray-50 shadow-sm">
                    <div className="flex flex-col sm:flex-row justify-between items-start">
                        <div className="flex-grow mb-3 sm:mb-0">
                            ...
                            <p className="text-xs text-gray-400 mt-2">בדיקה אחרונה: {alert.lastChecked ? formatRelativePostedDate(alert.lastChecked) : 'טרם נבדק'}</p>
                        </div>
